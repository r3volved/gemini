module.exports = async ( monitor ) => {
    ...
    let title = roled.filter(r => r).length 
        ? `Added ${roles.size} to ${roled.filter(r => r).length} users`
        : `No roles were added to any users`
    ...
    monitor.response = {
        ...
        embed:{
            title: title,
            description:[
                "*Note: I am only capable of applying roles available to me*"
            ],
            fields:[{
